...
const contrastCache = new Map<string, '#FFFFFF' | '#000000'>()
 ...
 */
export function useImageContrast(imageSrcGetter: () => string) {
  const fontColor = ref<'#FFFFFF' | '#000000'>('#FFFFFF')
  const isLoading = ref(false)

  const computeContrast = (src: string) => {
    if (!src) return
    ...
    img.onload = () => {
      try {
        ...
        const chosen: '#FFFFFF' | '#000000' = crWhite >= crBlack ? '#FFFFFF' : '#000000'
  ...
  const fallback = (src: string) => {
    ...
    let defaultColor: '#FFFFFF' | '#000000' = '#FFFFFF'
