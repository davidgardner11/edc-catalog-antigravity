...
- `tests/e2e/a11y.spec.ts` covers keyboard access (card -> dialog -> Escape), focus trapping, carousel/swatch buttons and runs axe-core via `@axe-core/playwright`; it fails on any `critical` violation inside the grid or the open dialog.
